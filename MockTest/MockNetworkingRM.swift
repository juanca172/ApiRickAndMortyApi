...
class MockNetworkingRM: XCTestCase {
    ...
    func testSuccesFullResponse() {
        ...
        //when
        let arrayAEncodiar = ["juan","camilo","fonseca"]
        let data = try? JSONEncoder().encode(arrayAEncodiar)
        
        MockURLProtocol.requestHandler = { request in
            guard let data = data else {
                fatalError("No hay data")
            }
            let response = HTTPURLResponse(url: apiURL, statusCode: 200, httpVersion: nil, headerFields: nil)!
            return  (response,data)
        }
        let apiRickAndMortyGeneric = ApiRickAndMortyGeneric(urlSession: urlSession , jsonDecoder: JSONDecoder())
        apiRickAndMortyGeneric.getData(urlRequest: request) { (result: Result<[String], RMError>) in
            switch result {
    ...
    func testFailureImposibleDecodiar () {
        //Given
